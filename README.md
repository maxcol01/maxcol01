## Hi, I'm Maxime 👋

Mechanical Engineer transitioning toward data science and AI-focused software development, with a focus on Python and web technologies.

### Interests
- Python programming for data science and machine learning
- Artificial Intelligence and applied machine learning
- Web development and AI-powered web applications

### Currently learning
- Data science and machine learning through formal training
- Foundations of deep learning and modern ML workflows

### Education & Training
- University of Liège - Phd and mechanical engineering degrees
- CS50 — Introduction to Computer Science, Introduction to Cybersecurity (in progress)
- Zero To Mastery - continuous learning
- Liora (ex-DataScientest) - Data Scientist path (https://liora.io/en/courses/data-ai/data-scientist) (in progress)
- Full-Stack Open (University of Helinski) - (scheduled 2027)

### Contact
- LinkedIn: https://www.linkedin.com/in/maxime-collet-aab39ba3/



<!---
maxcol01/maxcol01 is a ✨ special ✨ repository because its `README.md` (this file) appears on your GitHub profile.
You can click the Preview link to take a look at your changes.
--->
